...
import { _ApiProducts } from "../../../../api/products";
...
export default function Stock() {
  ...
  const GetDataStock = async () => {
    setLoading(true);
    try {
      const response = await _ApiProducts().Getlist(filters);
  ...
  const fetchDropdownData = async () => {
    try {
      const response = await _ApiProducts().DDL();
  ...
  const handleDelete = async () => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await _ApiProducts().Delete(bodyData);
